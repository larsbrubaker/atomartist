...
mod tests {
    ...
    /// is ~50 px. In turntable mode that should rotate the camera by
    /// ~0.5 rad of pitch — far from snapping.
    #[test]
    fn multi_step_drag_in_turntable_mode_does_not_snap() {
        ...
        let after_back = camera.lock().unwrap().orientation * glam::Vec3::Z;
        let after_elevation = after_back.z.clamp(-1.0, 1.0).asin();
        // 50 px * 0.01 rad/px = 0.5 rad of pitch. From the default
        // ~0.38 rad elevation, dragging cursor-up lowers elevation by
        // ~0.5 rad → ends near -0.12 rad. Still well above the lower
        // pole of -π·0.499.
        let delta = before_elevation - after_elevation;
        assert!(
            (delta - 0.5).abs() < 0.05,
            "10×5 px upward drags should pitch by ~0.5 rad; got {delta} rad ({} → {})",
            before_elevation, after_elevation
        );
    }
