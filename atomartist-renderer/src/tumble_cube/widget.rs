    ...
    fn on_mouse_move(&mut self, pos: Point) -> EventResult {
        ...
        match &mut self.drag {
            CubeDrag::Rotating { last_local } => {
                // Once the gesture is recognised as a drag, run the
                // exact same "standard" rotate path the main viewport
                // uses for orbit drags — no cube-specific math, no
                // ray/hit involvement. The cube has no world-space
                // pivot of its own, so the orbit pivots around the
                // shared camera's existing centre (i.e. it's a pure
                // rotation, no centre translation). Same scale as the
                // viewport (`viewport_widget_interactions::on_mouse_move`)
                // so cube and viewport drag at the same speed.
                ...
                *last_local = pos;
                {
                    let mut cam = self.inputs.camera.lock().unwrap();
                    let pivot = cam.center;
                    cam.orbit_drag_around(pivot, -dx * scale, dy * scale);
                }
