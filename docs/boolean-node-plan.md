...
|---|---|---|---|
| B-1 | Correctness floor | robust import + `status()` check surfaced as a node error; `MeshGL::merge` + seam-weld retry on NotClosed replaces `strip_normals`'s hand weld; `set_default_engine(Auto)` at startup; reproducing test for today's silent-garbage case first (it's a bug) | |
