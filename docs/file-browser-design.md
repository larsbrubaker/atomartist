...
| 6g-3 | Drag ghost + icon colors | The drag ghost is the item's rendered icon (ND: 48 px, the cached icon image; label-text fallback), attached to the cursor for the whole gesture including over the bed. Icon colors match ND's softer look: compare ND/MatterCAD default node colors and add the missing linear→sRGB handling in mesh_raster shading (ours reads over-saturated). *Outcome: node default colors were already byte-identical to ND (#FE8D86 box, etc. — the MatterCAD palette on both sides); the divergence was the sRGB output encode ND's icon renderer applies (`ColorManagement.enabled = false` skips input decode but not the output transfer). mesh_raster now encodes at pixel write — avg Box-icon saturation 0.15 → 0.07, lit faces read near-white. The ghost is the type's 48 px icon at 0.85 alpha centered on the cursor (glyph+label pill fallback). Two bugs fixed en route: the ghost froze over the bed (nothing requested frames once 6g-1 parked the loop — moves of a started gesture now request draws), and FloatingOverlayHost's bounds-only hit_test swallowed mid-drag presses (now defers to the child's own hit_test in child-local coords).* | **done** |

## 5d. Phase 6h — third feedback round (2026-08-15)

| # | Step | Contents | Status |
|---|---|---|---|
| 6h-1 | Icon lighting parity | Icons still read paler than ND's. Hypothesis to VERIFY against ND's vendored three.js source (three ~0.182, physical lights): light intensities pass through the Lambert BRDF's 1/π, so ND's effective rig is ≈ ambient 0.55/π, key 1.6/π, fill 0.5/π — much darker/more saturated than our straight lambert sum, then sRGB-encoded. Derive the exact math from their three build, re-shade, and compare rendered pixels numerically. *Outcome: hypothesis confirmed for icons — stock MeshLambertMaterial under r182 physical lights: `srgb_encode(tint·(0.55 + 1.6·dotNL_key + 0.5·dotNL_fill)/π)`, all six Box faces pinned byte-exact. The user's "we emulated the original GL pipeline" is true of the MAIN viewer (depth-peeling.js custom ShaderMaterial: gamma-space base, two camera-fixed lights 0.7/0.5 + 0.2 ambient, Blinn-Phong, no π, no output encode, LinearSRGBColorSpace) — but the icon renderer is its own stock WebGLRenderer and provably does not share it; both pipelines documented in mesh_raster.rs for the future bed-shading comparison.* | **done** |
| 6h-2 | Handle thumb visuals | Paint ONLY the small thumb (no full-height lane/separator column): ND's rounded protruding tab with chevron, hover highlight (accent fill like ND's blue), and the tooltip "Show library (drag to resize)" / "Hide library (drag to resize)". Hit-test already grip-only (6g-2); this is the visual + hover/tooltip parity. *Outcome: outer-edge rule gone; grip paints as ND's protruding right-rounded tab, accent fill + white chevron on hover (hover deliberately held during a resize drag), tooltip via agg-gui's wrapper-free `WidgetBase::tooltip` path. Being the app's first hover-latching widget it exposed that neither shell forwarded window-leave: demo-native now wires `CursorLeft` → `App::on_mouse_leave`, demo-wasm a `mouseleave` listener. favorites_bar.rs split events out (favorites_bar_events.rs); demo-native's winit translation helpers split to winit_input.rs for the 800 cap.* | **done** |
| 6h-3 | Node-canvas navigation controls | ND (`_graph_panel.ejs:5-23`, `graph-manager.js`): top-left of the node pane, two groups — home button (12,12) + a segmented 3-mode group at (64,12): Select (default) / Pan ("middle click") / Zoom ("scroll wheel"). Home = **fit-all-nodes, animated**: bounds = union of node AABBs incl. title bars, padding 50 graph-units both sides, `scale = min(w/(bw+100), h/(bh+100))` clamped 0.1..10, offset centers the bounds; 500 ms ease-in-out-cubic. Modes only change drag semantics: pan = background LMB-drag pans; zoom = LMB-drag `exp(−dy·0.005)` about the press point; select = rubber-band. Middle-drag always pans; wheel always zooms ×1.1 about the cursor. No persistence, no shortcuts. Styling: segmented buttons, hover lift, selected mode = primary-accent tint. *Outcome: GraphNavCluster overlay ("graph-nav") on the canvas pane; InteractionMode + the 500 ms fit tween + SetView live upstream in the editor (tween ticked from layout, cancelled by any user view input — drags and wheel always win, pinned failing-first). ND's top-down math translated to Y-up (zoom drag `exp(+dy·0.005)`); zoom clamp uses the editor's own 0.15..3.0. The cluster overlaps the starter graph's first title bar at default layout, same as ND.* | **done** |
| 6h-4 | View-state persistence | ND persists **per-design inside the graph JSON**, written only at save: `viewState {scale, offset[2]}`, `dividerPosition` (% of preview pane, 20..80), `cameraState {position, target, initialPosition, initialTarget}` (ortho saved as perspective-equivalent distance). Restore is deferred until layout settles; missing `viewState` leaves the canvas as-is (no fit), missing `cameraState` auto-frames on first geometry (`distance = maxDim·0.85/tan(fov/2)`, dir (−0.1, −1, 0.8)·d). View moves never mark the design dirty (stripped before change compare). Mirror in `.atmr` (pre-release: adopt outright); File → New resets camera-positioning only. *Outcome: optional `view {view_state, divider_position, camera_state}` on GraphFile, composed ONLY by `write_project_to_bytes_with_view` — `save_graph` always emits None so ChangeTracker can never see a view move (empty view is byte-identical to the old encoder). Restore rides the editor's command queue + SplitterRatio adoption on the first sized frame; a shared `camera_framed` slot both suppresses auto-frame after a restored camera and re-arms on File → New (fixing a latent bug where only the session's first project auto-framed — reproduced first). Degenerate/non-finite poses are refused and treated as no-camera; camera roll is lost on restore (two-point ND pose, level horizon — documented). Divider persists clamped 0.20..0.80 while the live drag range stays 0.05..0.95, rationale documented both sides.* | **done** |
