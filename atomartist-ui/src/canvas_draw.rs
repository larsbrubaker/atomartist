...
/// so light / dark mode toggles flow through automatically.
...
impl CanvasPalette {
    /// Build the palette from agg-gui's current visuals — adapts to light
    ...
    pub fn from_visuals(v: &agg_gui::theme::Visuals) -> Self {
        ...
        let canvas_bg = if dark {
            ...
            // Slightly cooler than panel_fill so the canvas reads as
    ...
    /// Backwards-compat shim used by existing call sites.
    pub fn dark() -> Self {
