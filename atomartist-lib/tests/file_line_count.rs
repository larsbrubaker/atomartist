...
//!
//! Static data — serialized save files, fixture documents, captured payloads —
//! is explicitly NOT in scope. Those files describe a single artifact that
//! must remain whole, and an AI assistant is not expected to read them
//! linearly the way it reads source. Add such directories to
//! `EXCLUDED_DIRS` rather than splitting their contents.
...
// external test suites, and static data fixtures. Do not exclude first-party
// source trees just because a file is generated-looking; if it is checked into
// a source/package directory and may be touched by an agent, it should be
// counted.
const EXCLUDED_DIRS: &[&str] = &[
    ...
    "demo-wasm/pkg",
    "reference",
    // Static save-file fixtures for the NodeDesigner importer. Each JSON
    // here is one serialized document and must stay intact; they are not
    // source that an agent should reformat or split.
    "atomartist-lib/tests/nodedesigner_examples",
];
